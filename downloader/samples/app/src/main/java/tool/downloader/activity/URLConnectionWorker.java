...
public class URLConnectionWorker implements Runnable {
    ...
    public void run() {
        ...
        while (requsetTimes >= 0 && requsetTimes < 3) {
            ...
            try {
                ...
                Log.d("FILEDOWNLOAD", "URL|" + downloadUrl);
                URLConnection urlConnection = downloadUrl.openConnection();
                conn = (HttpURLConnection) urlConnection;
                Log.d("FILEDOWNLOAD", "openConnection|" + conn);
//                conn.setInstanceFollowRedirects(false);
                conn.setConnectTimeout(CONNECT_TIME_OUT);
                conn.setReadTimeout(READ_TIME_OUT);
//                String range = "bytes=" + completeSize + "-" + totalSize;
                String range = "bytes=" + completeSize + "-" + (totalSize - 1);
                conn.setRequestProperty("Range", range);
                Log.d("FILEDOWNLOAD", "start connect|" + range);
                Log.d("FILEDOWNLOAD", "connectting..");
                int responseCode = conn.getResponseCode();
                Log.d("FILEDOWNLOAD", "getResponseCode|" + responseCode);
...
//                    throw new IllegalStateException(url + warnning);
                ...
                Log.d("FILEDOWNLOAD", "getInputStream|" + is);
                ...
                e.printStackTrace();
                notifyDownloadFailure(url, "Exception:" + e.getMessage());
                if (requsetTimes >= 2) {
